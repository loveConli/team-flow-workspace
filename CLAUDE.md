...
│   ├── architecture-api-db-design-enhancement-v0.7.md  # 设计增强方案 v0.7（orchestrator v2，历史参考）
│   ├── architecture-api-db-design-enhancement-v0.8.md  # 设计增强方案 v0.8（★ 当前权威版本，subagent 编排+状态治理）
...
模糊需求
  → [既有项目接入层 workflow-bootstrap]（一次性，已有 baseline.md 则跳过）
       B1 代码库侦察（recon-probe.sh 脚本 + codebase-recon-analyst 并行子代理）
  ...
  → S2 PRD + 原型阶段 → ce-brainstorm + 冻结前 PRD 完整性评审（prd-completeness-reviewer）
       + 原型循环上提（prototype 内部编排器，主代理只编排：env-scout→builder→reviewer→人工评审→冻结）
  → S3 计划阶段 → ce-plan（pipeline 快速路径，入口问定 plan_mode）→ plan.md（change 拆分+依赖+高阶技术方向，不含接口清单）
  ...
  → change 完成：arch-merge → prototype-sync（顺序提交，写入路由子代理）+ 复利晋升
```
...
| P3-2 | recon-probe.sh 并入 `ssf recon` CLI（当前为 bootstrap skill 独立脚本，超大仓库 LOC 性能可优化为按扩展名批量 xargs） | 🔲 待实施 | v1.0.0+ | 来源：v0.8 §18.4.2，v0.15.0 以独立脚本落地 |
