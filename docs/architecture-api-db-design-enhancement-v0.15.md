# 架构 / API / DB 设计增强方案 · v0.15（阶段同步 + 原型版本隔离 + 产品级架构设计增强）

> 版本：v0.15 · 重大修订（增量式，承袭 v0.14）
> 目标插件版本：**v0.37.0**
> 修订依据：v0.14 全量内容**继续有效**；v0.15 仅记录本次增量修订。
> 触发来源：LT 2026-08-05 团队协作诉求——工作流各阶段产物需 commit/push 同步供团队接力；原型独立仓库 + 多版本并行迭代需 worktree 隔离（详见 §68）。
> 决策确认：LT 2026-08-05 决策 ✅（详见 §69 决策落实状态）
>
> **v0.15 修订摘要（相较 v0.14）**：
