# 架构 / API / DB 设计增强方案 · v0.16（E2E case 记录与复利闭环 + test-merge 五 bug 修复）

> 版本：v0.16 · 重大修订（增量式，承袭 v0.15）
> 目标插件版本：**v0.38.0**
> 修订依据：v0.15 全量内容**继续有效**；v0.16 仅记录本次增量修订。
> 触发来源：workflow-feedback 2026-08-05（vrm4teamflow 项目）——① `tf test-merge` 五 bug（resolveDeferred 误删 / extractSummary bold 返回 0 / dry-run 真写 / 覆盖式失效 / 空列错位）② E2E case 未纳入 test-matrix 与复利台账——设计 v0.12 §42.4 已承诺 `test_tier` 三值（unit/integration/e2e），实现未落地（详见 §70/§71）。
> 决策确认：LT 2026-08-06 决策 ✅（详见 §71 决策落实状态）
>
> **v0.16 修订摘要（相较 v0.15）**：
...
## 六十八、阶段同步与原型版本隔离（v0.37.0）

> 触发来源：LT 2026-08-05 团队协作诉求。现状工作流各阶段产物只落本地工作区、无自动 commit/push；原型为独立仓库 + 多版本并行迭代，需 worktree 隔离。本版引入阶段同步门禁点 + 统一同步命令 + 原型版本隔离 + 制品链路径版本化修正。

### 68.1 现状缺口（实证）

| 缺口 | 证据链 | 后果 |
|------|--------|------|
| 工作流无自动 commit/push | 全 24 skill 的 SKILL.md **0 处** `git commit/push` 指令；workflow-orchestrator / workflow-start 全文 **0 处** git 关键词；代码层仅 arch-merge / test-merge 内嵌自动 commit（显式 CLI 触发，arch-merge.mjs:411-441），全库 **0 处** git push | 阶段产物（PRD/原型/orchestrator.yaml/changes/）只落本地 → 团队成员 `git pull` 拉不到 → S2→S3 接力断档 |
| 原型仓库分支隔离无代码强制 | 原型独立仓库（规范约定）+ 版本分支（`prd-vN`）仅文档声明（prototype/SKILL.md:88、prototype-env-scout.md:34-35），**无分支创建/切换/合并的代码机制** | 多版本并行迭代时单工作区切分支互相污染（git 未提交改动随切换迁移到目标分支）；团队无法并行维护 v1/v2/v3 原型 |
| 制品链引用路径假设失效 | 全制品硬编码相对路径 `prototype/`（spec-writer/SKILL.md:16-22、build-executor/SKILL.md:32、e2e/SKILL.md:53、prototype-sync.mjs:37），隐含"原型固定在项目根" | 原型迁入 worktree 后，实施阶段引用的 UI 契约指向错误位置（主干旧版本或空目录） |

### 68.2 五个同步门禁点

阶段完成时注入 **AskUserQuestion 阻塞确认**（复用既有编排层确认模式，s2-prd-prototype-loop.md:50），三选项：

- **A 提交并推送**：执行 `tf publish <target> --push`（含原型场景 push 对应版本分支）
- **B 仅提交不推送**：本地留档，团队暂不同步
- **C 暂不同步**：继续下一阶段，后续可补

| 门禁点 | 阶段 | 制品 | 同步对象 |
|--------|------|------|---------|
| G1 **S2 冻结后** | orchestrator S2 | prd/vN/ + 原型（push `prd-vN` 分支）+ orchestrator.yaml | 需求定义（团队讨论基线）|
| G2 **ARCH 完成后** | orchestrator ARCH | docs/architecture/iterations/vN/ + 评审 verdict | 架构决策（实施依据）|
| G3 **orchestrator 结束** | orchestrator S4 | changes/*/ + change-brief.md + change_dag | 任务分发（成员领取）|
| G4 **execution-contract 完成** | workflow-start bridging→approved-for-build | change 规划制品 4+1 | 契约（实施授权）|
| G5 **change closing** | release-archivist | 实施代码 + arch-merge 回写 + test-ledger | 落地结果（团队最需要看的）|

**密度约束**：仅上述 5 点。S3 plan 完成并入 G2（紧邻 ARCH）；原型补跑/重跑（v0.20.0 路径）并入 G1；S5 跨 change 监控非产物节点不设点。

### 68.3 统一命令 tf publish

```
tf publish <target> [--push] [--dry-run]
  target: --prd（PRD + 原型 + orchestrator.yaml）
        / --arch（架构快照 iterations/vN/）
        / --changes <dir>（change 制品）
        / --all（全部）
```

- **复用 arch-merge 白名单 commit 模式**（arch-merge.mjs:400-441）：只 add 本次 target 目录 + `detectOutsideDirty` 外部脏文件告警 + 独立写锁 `.publish.lock`（团队协作安全底线）
- **push 确认制**：`--push` 显式开启；默认**只提交不推送**（与门禁点 B 选项"仅提交不推送"一致）；`--dry-run` 仅预览；只推当前分支、不跨分支（push 是外向操作，强制确认）
- **命令名定稿**：`tf publish`（LT 2026-08-05 拍板，§69）；避开既有 `tf sync`（cmd-sync.mjs 已占用"specs 增量同步"语义）

### 68.4 原型版本隔离（worktree，与代码实施同构）

原型独立仓库（**有远程**，LT 确认 2026-08-05），复用 ensure-branch / deisolate 机制，**隔离单位 = PRD 版本**（非 change）：

```
<workspace>/.worktrees/                  # 统一隔离区（gitignored，与代码实施 ensure-branch 同布局）
├── <change-C1>/<repo>/                  # 代码实施 worktree（现状，ensure-branch.mjs:13）
└── prd-v2/<prototype-repo>/             # 原型版本 worktree（挂 prd-v2 分支，新增）
```
...
| 阶段 | 动作 | 说明 |
|------|------|------|
| 创建 | PRD vN 冻结（S2）时 `tf prototype branch <prd-vN>` | 从远程拉/创建 `prd-vN` 分支 + worktree；幂等（已有则复用）|
| 演进 | 版本迭代 + 各 change 的 prototype-sync 落在版本 worktree | 不与代码实施 worktree 混用 |
| 收尾 | **版本收尾 merge 回主干 + 清理 worktree**（方案 A，LT 决策 2026-08-05）| merge 时机 = change_dag 该版本全部 closing，一次 merge 干净 |
| 后续版本 | 从主干切新版本 worktree | 主干 = 完整原型工程真相源（含已收尾版本累积），后续对既有功能变更有完整参照 |

**与代码实施 ensure-branch 的关键差异**：代码实施隔离单位是 change、收尾时 merge 回主干删 worktree；原型隔离单位是 **PRD 版本**，merge 时机是**版本收尾**而非每个 change closing（避免冻结后 change 的 prototype-sync 反复修正导致主干多次 merge）。

### 68.5 制品链路径版本化修正

核心原则：**制品中的原型引用 = 语义相对 + 版本绑定**，禁止裸 `prototype/`。

| 制品 | 修正 |
|------|------|
| spec-writer 读原型 | 先解析版本 → 读 `prd-vN` worktree 的 `prototype/`；design.md/tasks.md 引用保留 `prototype/<rel>` 写法，但**顶部声明 PRD 版本 + 原型 worktree 路径**（溯源链清晰）|
| execution-contract UX 增量 | 段首注明目标 `prd-vN` worktree；`[ADD]/[MODIFY]` 相对路径语义不变（prototype-sync.mjs 的 `[ADD]/[MODIFY]` 解析）|
| build-executor UI 契约 | **实施阶段原型说明 = `prd-vN` worktree 目录下的原型**（LT 强调：非主干或固定路径）|
| e2e 原型期 | http-server 托管 `prd-vN` worktree 的 `prototype/`，baseURL 指向它（e2e/SKILL.md 复用机制段）|
| prototype-sync | `--prototype-dir` 缺省按版本解析 worktree（`resolvePrototypeDir`/`derivePrdVersion`：change-brief `upstream_plan_ref` → 版本 → `.worktrees/prd-vN/prototype/`；worktree 缺失回退主仓库 `prototype/` 并提示 `tf prototype branch <prd-vN>`）|

**版本推导链**（不新增状态机字段）：`change-brief.md` 的 `upstream_plan_ref: prd/vN/plan.md` → PRD 版本 vN → 原型 worktree 路径。

### 68.6 团队同步闭环

> **worktree 是本地工作区，不是同步载体；真正的同步载体是 worktree 挂的 `prd-vN` 分支**（push 到远程，团队可及）。`.worktrees/` 在 `.gitignore` 内（ensure-branch.mjs 的 ensureWorktreeGitignore），团队成员本地**没有**该目录，必须主动获取。

| 角色 | 动作 | 载体 |
|------|------|------|
| 迭代者（成员 A）| 原型迭代完成 → **push `prd-vN` 分支**（门禁点 G1 提醒）| 分支（入 git）|
| 查看者（成员 B）| `tf prototype branch <prd-vN>` 一键拉取（自动 fetch 远程分支建 worktree，幂等复用）| 命令化，免手动 `git worktree add` |

### 68.7 实现分层与版本范围（v0.37.0 P0）

| # | 内容 | 落点 |
|---|------|------|
| P0-1 | 原型版本 worktree 命令 `tf prototype branch <prd-vN>` / `tf prototype deisolate <prd-vN> [--merge] [--clean]`（复用 ensure-branch/deisolate 模式，参数化：目标仓库/分支名 `prd-vN`/merge 时机=版本收尾）| 新建 cmd-prototype.mjs |
| P0-2 | 统一同步命令 `tf publish`（复用 arch-merge 白名单 commit + 脏文件检测 + 写锁 + push）| 新建 scripts/lib/cmd-publish.mjs |
| P0-3 | 5 门禁点注入（AskUserQuestion 三选项）| workflow-orchestrator references（s2-prd-prototype-loop / s3.5 / s4-split-validate）+ workflow-start（DP-3）+ release-archivist |
| P0-4 | 制品链路径版本化修正（spec-writer / contract-builder / build-executor / e2e / prototype-sync）| 对应 SKILL.md + prototype-sync.mjs `--prototype-dir` 默认值 |
| P0-5 | v0.15 文档 + Roadmap 登记 + CLAUDE.md 权威版本指针 | docs/ |
...
| 决策点 | 决策 | 落实版本 | 状态 |
|--------|------|---------|------|
| 原型仓库形态 | 独立仓库 + 有远程（LT 确认 2026-08-05）| v0.37.0 | 待实施 |
| 原型隔离机制 | worktree 复用 ensure-branch/deisolate，隔离单位 = PRD 版本 | v0.37.0 | 待实施 |
| 原型 merge 时机 | 版本收尾（change_dag 该版本全部 closing）一次 merge 回主干（方案 A，LT 决策 2026-08-05）| v0.37.0 | 待实施 |
| 同步命令名 | `tf publish`（LT 2026-08-05 拍板；避开 `tf sync` 冲突——cmd-sync.mjs 已占用 specs 语义）| v0.37.0 | 已定 ✅ |
| push 策略 | `--push` 显式开启 + `--dry-run` 默认 + 只推当前分支（外向操作强制确认）| v0.37.0 | 待实施 |
| 制品链引用 | 原型引用 = 语义相对 + 版本绑定；版本从 change-brief.upstream_plan_ref 推导，不新增状态字段 | v0.37.0 | 待实施 |
| 同步门禁点密度 | 5 点（G1-G5），S3/原型补跑/S5 不单列 | v0.37.0 | 待实施 |
...
| push 卷走他人未提交工作（v0.37.0）| 白名单 git add（复用 arch-merge）+ 脏文件告警 + push 确认制（`--push` 显式 + 只推当前分支）|
| worktree 目录团队不可见（v0.37.0）| 分支才是同步载体；`tf prototype-ensure-branch` 一键拉取；门禁点提醒 push `prd-vN` 分支 |
| 多版本并行 worktree 冲突（v0.37.0）| 按 PRD 版本隔离（`prd-vN` 唯一 worktree），不按 change 隔离；与代码实施 worktree 分区（`.worktrees/prd-vN/` vs `.worktrees/<change>/`）|
| 同步命令命名冲突（v0.37.0）| 避开既有 `tf sync`（specs 语义），候选 `tf publish`/`tf share`/`tf stage-sync`，实施时拍板 |
| 原型版本分支与主干漂移（v0.37.0）| 版本收尾一次 merge 回主干 + 收尾前团队看 `prd-vN` 分支；主干 = 完整原型真相源，防在途半成品入主干 |

---

## 七十、test-merge 五 bug 修复（v0.38.0）

> 来源：workflow-feedback 2026-08-05（P1-1~P1-4），vrm4teamflow 项目 C1 复利回写时暴露。顶层 `main()` 自调用崩溃已在 v0.35.0 修复；本版本修复剩余五个问题。

### §70.1 resolveDeferred 误删 Current Cases（最严重）

- **现象**：`tf test-merge` Step 3 resolveDeferred 用全文件 regex `\|<caseId>\|[^\n]*\n` 删除匹配行，把 baselines `Current Cases` 表正常 case 行全部误删（表体清空、Evolution Log 计数保留）。
- **根因**：regex 全文件匹配（未限定 Deferred Items 段）；且 newCaseIds 收集 test-matrix 全部新 case，mergeBaselines 刚把这些 case 写入 Current Cases → 必然命中误删。
- **修复**：逐行扫描，仅删除 `## Deferred Items` 段（到下一个 `## ` 标题之间）内被新 case 覆盖的行。
- **实证**：修复前 Current Cases 表体清空；修复后 Current Cases 保留、Deferred 被覆盖行删除（tests/lib/test-merge.test.mjs 覆盖）。

### §70.2 extractSummary 遇 markdown bold 返回 0

- **现象**：C1 矩阵 Summary 为 `- Total cases: **87**`（LLM 手写加粗），regex `Total cases:\s*(\d+)` 遇 `**` 匹配失败 → 统计显示 0。
- **修复**：regex 容忍可选 `**` 前缀（`\s*\*{0,2}\s*`），普通数字与 bold 格式均正确解析；新增 `E2E cases:` 行识别（与 §71 联动）。

### §70.3 dry-run 实为真写

- **现象**：`dryRun` 仅在 Step 6 gitCommit 生效，Step 2-5 照常写盘 → dry-run 污染真实 ledger；dry-run + 正式各 append 一次 → Evolution Log 重复。
- **修复**：mergeBaselines / resolveDeferred / appendChangelog / rewriteIndex 全链路透传 dryRun，dry-run 不写任何文件（返回 would-create/would-update 预览）。

### §70.4 mergeExistingBaseline 覆盖式失效

- **现象**：`line.startsWith('')` 恒真 → Current Cases"当前态覆盖式"语义失效，旧 case 行每次 merge 累积。
- **修复**：退出 inCases 仅由 `## ` 标题触发；Current Cases 表 = 表头 + 新 case 行（旧行跳过）。

### §70.5 `.filter(Boolean)` 空列错位

- **现象**：cells 提取用 `filter(Boolean)` 丢空列，mock 等空列导致后续列索引左移，work_mode/test_file/test_method_name 静默错位（E2E case 的 mock 列必空，高发）。
- **修复**：改 `split('|').slice(1,-1)` 按索引保留空列。

### §70.6 Evolution Log 幂等

- **现象**：同 change 同日重复执行 test-merge → Evolution Log 重复 append。
- **修复**：append 前检查 `**{date}** [{changeName}]:` 是否已存在。

---

## 七十一、E2E case 记录与复利闭环（v0.38.0）

> 来源：workflow-feedback 2026-08-05（P2-1）。设计 v0.12 §42.4 已定义 `test_tier` 三值（unit/integration/e2e），但实现仅落地 unit/integration，E2E case 无法进入 test-matrix 与复利台账，含 UI/跨层 AC 的前端回归覆盖在全局不可见。**本质是"实现补齐设计承诺"**。

### §71.1 test-matrix 支持 E2E case 记录

- **test_tier=e2e**：E2E case 沿用标准 12 列格式，`test_tier=e2e`；case_id 用 `{Module}-e2e-{scene}-{NNN}`（3 连字符 + 数字结尾，满足复利台账计数 regex）；mock 列填 `-`；test_kind 用 `playwright_prototype` / `playwright_integration`。
- **policy（test-strategy §3）**：从"e2e 独立覆盖不计入本矩阵"改为"含 UI 交互/跨层链路 AC 显式评估 E2E 必要性——纳入或记录排除理由"；金字塔分母 = unit+integration（e2e 不稀释）。
- **生成器（contract-builder）**：Generation Protocol 第 7 步 E2E 层评估；Summary/契约段加 `E2E cases: {N}`。
- **上游（spec-writer）**：Scenario 测试标签补 `##### E2E:`（可选）。
- **导出（test-matrix-export）**：`TEST_KIND_TO_TIER` 加 playwright 两类 → e2e。

### §71.2 复利回写 E2E 层级

- `tf test-merge` 的 mergeBaselines 用 cells[6]（test_tier）透传，E2E case 自动进 baselines；rewriteIndex 的 `test_tier_breakdown` 增加 e2e 统计（`unit:N, integration:N, e2e:N`）。
- extractSummary 识别 `E2E cases:` 行，Summary 输出含 e2e 计数。

### §71.3 下游 E2E 确认（release-archivist Step 5b）

检测 test-matrix 是否含 `test_tier=e2e` case：
- 无 → 跳过
- 有 + 已有 e2e 报告 → 折进验证（4 级产物验证 + 分级门禁）
- 有 + 无报告 → **AskUserQuestion 确认**："test-matrix 含 {N} 个 E2E case，是否执行 E2E 测试？"
  - 是 → 引导 `/e2e`（AC 提取 → Playwright → 报告 `docs/statistics/YYYY-MM-DD-e2e-report.md` 回填）
  - 否 → 记录 E2E skip + 理由，报告行 `| E2E | SKIP | reason |`（不 FAIL）

### §71.4 LLM 消费点 e2e 豁免（防断链）

三个消费点在矩阵含 E2E case 时会误判 FAIL，需豁免：
- **release-archivist Step 2b**：e2e case 从 coverage 分母豁免，单列 `e2e cases: implemented N/M（经 Step 5b 验证）`
- **build-executor implementer**：`test_tier=e2e` 不在 implementer 职责内（e2e skill / Step 5b 覆盖），不计模块 coverage
- **code-reviewer Step 5b**：coverage 分母排除 e2e；金字塔以 unit+integration 为分母

### §71.5 制品链交叉确认结论

- **无代码级断链**：guard checks（ready/complete 只查存在 + 全文件 hash）、cmd-validate、hash.mjs、s1-path-router（只读 coverage_status 语义）、solutions-promote 均不受影响。
- **格式纪律**：e2e case 必须标准 12 列（test_tier 第 7 列）；不得复用 tests/e2e/test-matrix.md 的变体列序（会导致 test-merge 按位拷贝串列）。

### §71.6 决策落实状态（v0.38.0）

| 决策点 | 决策 | 版本 | 状态 |
|---|---|---|---|
| E2E case 表示 | 统一 12 列 + test_tier=e2e（非独立 E2E 段） | v0.38.0 | 已实施 ✅ |
| 下游确认点 | release-archivist Step 5b（非 build-executor） | v0.38.0 | 已实施 ✅ |
| E2E 执行方式 | 引导 `/e2e`（用户手动，非自动调度） | v0.38.0 | 已实施 ✅ |
| E2E 层评估责任 | contract-builder 显式评估（纳入或记录排除理由） | v0.38.0 | 已实施 ✅ |
