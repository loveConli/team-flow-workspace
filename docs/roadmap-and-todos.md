...
| v0.41.0 | **workflow-bootstrap CLAUDE.md 初始化 + Git 多子仓库识别** | ① B4.5 CLAUDE.md 初始化（参考 Claude Code /init，索引级+结构级：项目概况/开发命令/工作区结构/Git 结构 + team-flow 产物结构索引；新建/追加双模式）② CLAUDE.md 标准模板 templates/claude-md-team-flow.md（覆盖 /init 内容 + 产物结构表）③ recon-probe.sh ⑧ Git 子仓库扫描（嵌套仓库/submodule/.gitignore 忽略规则，JSON 新增 git_structure）④ codebase-recon-analyst git-structure 维度（6→7 维度）⑤ B1.3 维度闭合校验（handoff status + deliverable 非空，缺失标 INCOMPLETE）；来源：LT 需求 2026-08-07 | ✅ 2026-08-07 |
| v0.42.0 | **B5 降级路径设计 + session-start 语言要求** | ① B5 重构两组降级选项（第一组确保产物可信：A 逆向重建按复杂度分级推荐 / B 交叉验证主代理机械比对产出 cross-check-report.md / C 真实性核对用户确认门；第二组直接进入：D-F 原 A-C 平移）② references/b5-path-decision.md（B 比对清单 + architecture 显式排除理由 + C 双模式核对协议）③ hooks/session-start msg 追加 Language 指令（中文交流，技术术语/代码/命令保英文）④ b1-reconnaissance Success Output 同步；来源：LT 需求 2026-08-07 | ✅ 2026-08-07 |
| v0.43.0 | **ce-brainstorm 阶段断点控制 + L1-L3 用户定义 + prd/→requirement/ 路径收敛** | ① FB1 阶段断点控制（Execution Architecture + BP-1~4 断点 + PRD 撰写委托子代理）② FB3 Phase 1.6 L1-L3 用户定义（Step 0 询问 + 两阶段确认）③ FB2 prd/ → requirement/ 路径收敛（新项目只建 requirement/vN/，50+ 文件路径更新，存量 prd/ legacy fallback）；来源：workflow-feedback 2026-08-07（3 条 P2 反馈） | ✅ 2026-08-07（xiaoyifeng 提交，npm 未单独发布，随 v0.43.1 发布） |
| v0.43.1 | **执行/关闭阶段质量保障闭环补强** | ① tf deisolate --merge 健壮性增强（dirty 阻断 / ahead 短路 / 冲突报告 / --force / 非零 exit）② release-archivist Step 5c Code Landing（代码合并 + 主分支编译验证 + 依赖链顺序）+ references/worktree-merge.md ③ 文件存在性双层防护（build-executor 完成前验证 + code-reviewer Step 1b）④ 测试执行可信度（Wave Verify 实际执行数 + Test Infrastructure Check）⑤ spec-writer specs/ 路径契约；来源：vrm4teamflow workflow-feedback 2026-08-06（20260806 批 6 条缺口）；设计增强方案 v0.18 §74-§77 | ✅ 2026-08-07 |
...
| P1-51 | workflow-start 编排流程修正（v0.39.1，v0.17 §72）：① workflow-start 从"一次性安排 + 被动等待"改为"主动串行编排"——每个 wave 完成后主动 dispatch code-reviewer ② build-executor prompt 修正——移除"After every wave, write a review report and record receipt"，改为通知 workflow-start ③ workflow-start 产物所有权约束强化——禁止直接修改子代理代码，review findings 处理标准化 ④ 修复后 re-review 流程——修复后必须 re-review 确认修复有效，记录 review receipt ⑤ closing 前置条件检查——review receipt 存在且 verdict=pass 才能进入归档 ⑥ 验证 hotfix/tweak 模式不受影响 | ✅ v0.39.1（2026-08-06） | v0.39.1 | 来源：vrm4teamflow workflow-feedback 2026-08-06（C2-policy-management 5 wave 未执行代码审查 + 产物所有权违反 + 修复后缺少 re-review）；设计增强方案 v0.17 §72 |
| P1-52 | `tf deisolate --merge` 健壮性增强（v0.43.1，v0.18 §74）：① dirty 阻断——worktree 未提交改动默认不合并（--force 跳过）② ahead==0 短路——无新 commit 跳过（幂等）③ 冲突报告——merge 失败解析 unmerged 文件清单 + 非零 exit code ④ 目标分支明确 | ✅ v0.43.1（2026-08-07） | v0.43.1 | 来源：vrm4teamflow workflow-feedback 2026-08-06（20260806-180000 + 180100）；设计增强方案 v0.18 §74 |
| P1-53 | release-archivist 新增 Step 5c Code Landing（v0.43.1，v0.18 §74）：检测 worktree 未合并 commit → AskUserQuestion 确认合并时机（A 现在合并 / B 暂不合并 / C 无需合并）→ 合并后主分支编译验证 + 依赖链顺序提示 + 冲突处理引导 | ✅ v0.43.1（2026-08-07） | v0.43.1 | 来源：vrm4teamflow workflow-feedback 2026-08-06（20260806-180000）；设计增强方案 v0.18 §74 |
| P1-54 | worktree 合并规范文档化（v0.43.1，v0.18 §74）：references/worktree-merge.md——merge commit 模式（不 rebase，强壮性优先）、多仓库策略（Case A 逐 repo）、依赖链顺序（C1→C2）、冲突处理（人工介入 + git add/commit）、合并后编译验证 | ✅ v0.43.1（2026-08-07） | v0.43.1 | 来源：vrm4teamflow workflow-feedback 2026-08-06（20260806-180100）；设计增强方案 v0.18 §74 |
| P1-55 | build-executor wave VERIFY 检查实际执行测试数量（v0.43.1，v0.18 §76）：用 `tf test record` 解析真实 `Tests run: N`，对照 test-matrix 预期用例数，<70% → 警告 + 调查（@Nested 静默跳过等），未查明不得报告 "N tests pass" | ✅ v0.43.1（2026-08-07） | v0.43.1 | 来源：vrm4teamflow workflow-feedback 2026-08-06（20260806-180500）；设计增强方案 v0.18 §76 |
| P1-56 | build-executor 测试框架兼容性检查（v0.43.1，v0.18 §76）：首 wave 前检测 pom.xml surefire 版本 + JUnit 5 @Nested 兼容性（surefire<2.22.2 警告），结果沉淀 conventions 供后续 change 复用 | ✅ v0.43.1（2026-08-07） | v0.43.1 | 来源：vrm4teamflow workflow-feedback 2026-08-06（20260806-180500）；设计增强方案 v0.18 §76 |
...
| P2-48 | build-executor 任务完成前文件存在性验证（v0.43.1，v0.18 §75）：Create 任务文件必须存在非空（test -s）、Modify 任务必须有 diff，验证失败不标记完成——"标记完成"与"文件存在"绑定 | ✅ v0.43.1（2026-08-07） | v0.43.1 | 来源：vrm4teamflow workflow-feedback 2026-08-06（20260806-180300）；设计增强方案 v0.18 §75 |
| P2-49 | code-reviewer 文件存在性审查维度（v0.43.1，v0.18 §75）：Step 1.5 前置检查——tasks.md File Structure 的 Create 文件必须存在非空（missing-file Critical）、Modify 文件必须有 diff（no-change Critical），spec-compliance 前置 | ✅ v0.43.1（2026-08-07） | v0.43.1 | 来源：vrm4teamflow workflow-feedback 2026-08-06（20260806-180200）；设计增强方案 v0.18 §75 |
| P2-50 | spec-writer 明确 specs/<capability>/spec.md 子目录契约（v0.43.1，v0.18 §77）：每个 capability 一个子目录，禁止 flat 文件 specs/<capability>.md（tf validate 拒绝） | ✅ v0.43.1（2026-08-07） | v0.43.1 | 来源：vrm4teamflow workflow-feedback 2026-08-06（20260806-180400）；设计增强方案 v0.18 §77 |
