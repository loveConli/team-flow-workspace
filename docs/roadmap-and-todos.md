...
| P1-51 | workflow-start 编排流程修正（v0.39.0，v0.17 §72）：① workflow-start 从"一次性安排 + 被动等待"改为"主动串行编排"——每个 wave 完成后主动 dispatch code-reviewer ② build-executor prompt 修正——移除"After every wave, write a review report and record receipt"，改为通知 workflow-start ③ workflow-start 产物所有权约束强化——禁止直接修改子代理代码，review findings 处理标准化 ④ 修复后 re-review 流程——修复后必须 re-review 确认修复有效，记录 review receipt ⑤ closing 前置条件检查——review receipt 存在且 verdict=pass 才能进入归档 ⑥ 验证 hotfix/tweak 模式不受影响 | 🔲 待实施 | v0.39.0 | 来源：vrm4teamflow workflow-feedback 2026-08-06（C2-policy-management 5 wave 未执行代码审查 + 产物所有权违反 + 修复后缺少 re-review）；设计增强方案 v0.17 §72 |
